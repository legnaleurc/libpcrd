...
DigitList = list[tuple[int, numpy.ndarray]]
...
def load_input(input_path: str) -> list[numpy.ndarray]:
...
def load_items() -> list[tuple[str, numpy.ndarray]]:
...
def get_item_count(source: numpy.ndarray, item: numpy.ndarray, digit_list: DigitList) -> int:
    ...
    display: numpy.ndarray = source[min_y:max_y, min_x:max_x]
    ...
    mask: numpy.ndarray = cv2.inRange(display, lower, upper)
    # invert color
    mask = ~mask

    # split digits
    _y, x, _ = display.shape
    grid_list: list[numpy.ndarray] = [mask[:, x - 19 * (i + 1):x - 19 * i] for i in range(4)]
